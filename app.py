        ...
        if not raw_news_text:
            ...
            with st.spinner('編輯台長官審稿中...正在生標題與稿頭...'):
                try:
                    ...
                    1. 【撰寫主播稿頭（Lead）】：
                       - 任務：濃縮這則新聞的最核心精華，寫成一段讓主播在畫面上唸出來的引言。
                       - 字數絕對限制：長度「絕對不可以超過 100 個字」。
                       - 符號嚴格要求：稿頭內的所有標點符號，請「一律使用半形符號」（如半形逗號,、句號.），絕對不要出現任何全形符號。
                       - 風格要求：口語化、通順易讀。【嚴格禁止超譯、腦補或誇大數據，必須 100% 貼合原文事實】。

                    2. 【下 3 個電視新聞大標題（CG Headline）】：
                       - 任務：參考台灣電視新聞的鏡面標題邏輯，抓出新聞重點或受訪者金句。【絕對禁止超譯、滑坡推論或使用原文未提及的強烈情緒字眼】。
                       - 字數絕對限制：每個標題的字數必須嚴格介於「17 到 20 個字」之間。
                       - 斷句與標點嚴格要求：標題內「絕對不可以」出現句號與逗號。斷句請直接使用「半形空白」，且【每個標題最多只能出現一次空白】。可適度使用半形的「!」或「?」增添語氣，但不可濫用。
                       - 受訪者金句排版鐵律：3 個標題中，【至少要有 1 個標題】必須引用核心受訪者的回答，且金句必須放在標題的【後半段】（半形空白之後）。
                       - 標題引述格式與字數榨取鐵律：
                         A. 只要標題有引述人物說法，必須嚴格使用「人物:」的格式（冒號請用半形 `:`，回答金句直接接在冒號後面）。
                         B. 同一個標題中，人物的「稱謂」與「姓名」【只能擇一出現，絕對不可以同時寫出】。禁用「立委黃國昌:」，只能精簡為「立委:」或「黃國昌:」。
                    ...
                    # 【關鍵優化】將溫度降到絕對冰點 0.0，徹底封印 AI 的創作與腦補神經
                    safe_config = genai.GenerationConfig(temperature=0.0)
