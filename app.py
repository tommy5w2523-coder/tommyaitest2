            ...
            with st.spinner('編輯台長官審稿中...正在生標題與稿頭...'):
                try:
                    ...
                    2. 【下 3 個電視新聞大標題（CG Headline）】：
                       ...
                       - 斷句與標點嚴格要求：標題內「絕對不可以」出現句號與逗號。斷句請直接使用「半形空白」，且【每個標題最多只能出現一次空白】。可適度使用半形的「!」或「?」增添語氣。
                       - 受訪者金句排版鐵律：3 個標題中，【至少要有 1 個標題】必須引用核心受訪者的回答，且金句必須放在標題的【後半段】（半形空白之後）。
                       - 標題引述格式與字數榨取鐵律：
                         A. 只要標題有引述人物說法，必須嚴格使用「人物:」的格式（冒號請用半形 `:`，回答金句直接接在冒號後面）。
                         B. 為了節省電視鏡面空間，同一個標題中，人物的「稱謂（如：立委、部長）」與「姓名（如：黃國昌）」【只能擇一出現，絕對不可以同時寫出】。禁用「立委黃國昌:」，只能精簡為「立委:」或「黃國昌:」。
